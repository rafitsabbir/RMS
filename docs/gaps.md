...
| G14 | Position / Language | Master data | Silent failure | Partial | `PositionDaoImpl.java:70-78`, `LanguageDaoImpl.java:70-78`: a duplicate name only reaches a `log.warn` (was `System.out.println` before Phase 1). Save always redirects (`PositionController.java:43`, `LanguageController.java:52`). The alerts in `createposition.jsp:26-45` and `createlanguage.jsp:27-46` (`errorMessage`/`successMessage`) are never set | Med | S | Confirmed |
| G15 | Position / Language | Master data | Partial flow | Partial | `PositionDaoImpl.java:95`, `LanguageDaoImpl.java:87`: `queryForObject` with a missing key throws an unhandled error (HTTP 500); there's no `@ExceptionHandler` or `@ControllerAdvice` | Low | S | Confirmed |
...
| G17 | Position / Language | Master data | Partial flow | Partial | Inserts set `isActive=1` (`:72` in both DAOs) and lists filter on `isactive=1` (`:23`), but delete removes the row (`PositionDaoImpl.java:28`, `LanguageDaoImpl.java:30`), through a GET (`PositionController.java:68`, `LanguageController.java:68`) | Med | M | Likely: soft-delete intent inferred |
| G18 | Position / Language | Master data | Partial flow | Partial | `PositionDaoImpl.java:51` `updatePosition` and `LanguageDaoImpl.java:91` `updateLanguage` don't check for duplicates; only add does | Low | S | Confirmed |
...
| G24 | Build | Security | Partial flow | Partial (Phase 1 on `dev`, not released) | Was: Spring 4.3.0 and Connector/J 5.1.36, plus three abandoned Dependabot branches. Phase 1 (2026-09-26) moved to the Spring BOM 5.3.39, `com.mysql:mysql-connector-j` 8.2.0, SLF4J/Logback and current front-end libraries with SRI (`pom.xml:13,32-39,93-116`), and deleted the three branches. Not released: the DAO tests, smoke test and acceptance checklist still have to pass, and the container's JNDI `driverClassName` should change. GitHub reported 26 alerts on `master` before this (2 critical, 9 high, 12 moderate, 3 low; external figure). Spring 5.3 itself is out of open-source support, so Phases 2–3 follow | High | M | Confirmed |
...
| G27 | UI | Security (output escaping) | Partial flow | Partial | Output isn't escaped: EL at `viewposition.jsp:47` and `viewlanguage.jsp:47`; scriptlets at `main.jsp:81,84,87` and `viewmarks.jsp:74-76`. A script saved in a position or language name runs in other users' browsers (stored XSS); upper-casing (`PositionDaoImpl.java:66-67`) doesn't stop HTML tags. Reachable without login because of G11 | Med | S | Confirmed |
...
| G24 | Security upgrades | Phased per [modernization-plan.md](modernization-plan.md):<br>• ~~Spring 5.3.39 via the Spring BOM as a waypoint (Java 8, `javax`)~~ done on `dev` (Phase 1)<br>• JDK 21<br>• the target, Spring 7.0 + Jakarta + Tomcat 11<br>• ~~`com.mysql:mysql-connector-j`~~ 8.2.0 (last release supporting MySQL 5.7) done on `dev`; the container JNDI `driverClassName` should still change<br>~~Close the three Dependabot branches~~ done 2026-09-26 | G22, G23 (Phase 0 safety net) | `WebMvcConfigurerAdapter` must go before 6.x (done in Phase 1); the container's JNDI config changes; the JDK must not move before Spring (Spring4Shell) | M |
