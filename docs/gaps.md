...
Last updated: 2026-09-25 (re-verified against `dev`; G26–G31 added)
...
| G11 | All controllers | Access control | Missing link | Not implemented | `WebConfig.java` has no `addInterceptors` and there's no filter; no controller reads the `user` session attribute (`rms/controller/*`). Every URL can be opened directly | High | S | Confirmed |
...
| G13 | Login | Authentication | Partial flow | Partial | `LoginDaoImpl.java:19` compares the raw password in SQL; there's no hashing in `src/main/java` | High | M | Confirmed |
...
| G22 | Data | All flows | Missing link | Not implemented | Tables `users`, `admin` (`LoginDaoImpl.java:19-20`), `position`, `language`, `candidate` and `marks` (`MarksDaoImpl.java:19-24`) have no DDL, migration or seed data; there's no `.sql` file or `src/main/resources` | High | M | Confirmed |
| G23 | All | All flows | No tests | Not implemented | No `src/test/` and no test dependency in `pom.xml` | Med | M | Confirmed |
| G24 | Build | Security | Partial flow | Partial (abandoned) | Three unmerged Dependabot branches (2022-06-21, 2022-12-10, 2022-12-16; `git for-each-ref refs/remotes`); `pom.xml:13,69` still on Spring 4.3.0 and Connector/J 5.1.36. GitHub's push message reported 26 known security issues on `master` (external figure, not verifiable from the code) | High | M | Confirmed |
...
| G28 | Position / Language | Master data | Partial flow | Partial | No `@Valid` or `BindingResult` (`PositionController.java:33-45`, `LanguageController.java:42-54`); blank names are uppercased and saved (`PositionDaoImpl.java:62-73`, `LanguageDaoImpl.java:62-73`) | Low | S | Confirmed |
| G29 | Build | Deployment | Dead code | Unused | `target/` is tracked in git with no `.gitignore`. The committed `target/rmsv2-1.0.1-SNAPSHOT.war` has no Position classes or JSPs (`unzip -l`), so it was built before `e5705c0` and is out of date | Low | S | Confirmed |
...
## Counts
- **By type (31 total):** Partial flow 14 · Missing link 8 · Dead code 5 · Stub 2 · Silent failure 1 · No tests 1
- **By impact:** High 10 · Med 10 · Low 11
- **By confidence:** Confirmed 24 · Likely 7

## By business capability
- **Recruitment pipeline:** G1, G2, G3, G4, G5–G7, G8, G9, G10
- **Access & security:** G11, G12, G13, G24, G26, G27, G30
- **Master data (Position/Language):** G14, G15, G17, G18, G28
- **Platform / UI:** G16, G19, G20, G21, G22, G23, G25, G29, G31
...
0. **Unblock:**
   ...
   - G29: rebuild or stop tracking `target/`.
1. **Quick wins (S each, no domain input needed):** G11, G27, G12, G14, G15, G16, G18, G26, G28, G30, G20/G21, G25, G31.
