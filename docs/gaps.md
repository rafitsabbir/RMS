...
| G35 | Build | Build portability | Missing link | Not implemented | `pom.xml:74-88` has no `maven-compiler-plugin` and no `maven.compiler.source`/`target`. Maven 3.6 then defaults to Java 1.5, which JDK 9 and later reject, so the build only works on JDK 8 | Low | S | Confirmed |
...
0. **Unblock:**
   ...
   - G35: set the compiler level so the project builds on current JDKs.
