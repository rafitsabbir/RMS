...
| Front-end libraries (CDN, every link pinned with an SRI `integrity` hash) | Bootstrap 3.4.1 and 4.6.2 (jsDelivr), jQuery 3.7.1 (code.jquery.com), DataTables 1.13.11 (the last 1.13 release: 1.13.12 returns 404 on cdn.datatables.net, 2026-09-26), Font Awesome 4.7.0 (cdnjs). Upgraded in Phase 1 from Bootstrap 3.3.7/4.1.1, jQuery 3.2.1/3.3.1 and DataTables 1.10.19 | as listed | `login.jsp:13-18`, `create*.jsp:10-14`, `view*.jsp:12-21`, `main.jsp:26` |
...
| Database driver | MySQL Connector/J `com.mysql:mysql-connector-j`, with `protobuf-java` excluded (only needed for the X DevAPI; 8.2.0 would pull `protobuf-java` 3.21.9, per its pom). Driver class `com.mysql.cj.jdbc.Driver`. 8.2.0 is the newest release that supports MySQL 5.7 servers; 8.3.0+ need 8.0+ (release notes). Chosen while the server version is unknown (open question #16) | 8.2.0 (was `mysql:mysql-connector-java` 5.1.36 until Phase 1) | `pom.xml:93-104` |
| Messaging/Integration | Not found | — | `pom.xml` |
| Scheduling/Batch | Not found | — | `src/main/java` (no `@Scheduled`) |
| Reporting | Not found | — | `pom.xml` |
| Logging | SLF4J API with Logback (console appender). Spring 5 logs through `spring-jcl`, which picks up SLF4J. (`spring-jcl-5.3.39.jar` contains `LogAdapter$Slf4jAdapter`; `logback-classic-1.3.16.jar` registers itself as the SLF4J provider and also as a `javax.servlet.ServletContainerInitializer`, `LogbackServletContainerInitializer`, which stops Logback cleanly on undeploy). One `System.out` remains in `main.jsp:172` (G30) | SLF4J 2.0.20, Logback 1.3.16 | `pom.xml:106-116`, `src/main/resources/logback.xml`, `PositionDaoImpl.java:22,73`, `LanguageDaoImpl.java:23,73` |
