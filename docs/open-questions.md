...
7. **Dependabot upgrades:** should the three unmerged branches (MySQL 8.0.28, spring-web 6.0.0, spring-webmvc 5.2.20) be merged? GitHub's push message reported 26 known security issues on `master`; this is an external figure that can't be verified from the code. spring-web 6.0.0 on its own wouldn't be compatible with Spring 4.3 (`git branch -r`, `pom.xml`). See G24 in [gaps.md](gaps.md).
